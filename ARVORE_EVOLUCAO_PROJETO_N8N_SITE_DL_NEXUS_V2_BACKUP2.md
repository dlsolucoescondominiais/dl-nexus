...
**Objetivo:** Documentar a arquitetura atual, o status operacional e a evolução planejada do ecossistema inteligente DL Nexus, abrangendo automações, site, segurança e integrações (corporativo e condominial).
...
*(Nota de Linguagem Comercial: Termos internos como "B2B" são apenas nomenclatura de sistema. Publicamente usamos "corporativo e condominial").*
...
- 🟢 **052_ANINHA_SOCIAL_RESPOSTA_META:** Engrenagem de disparo de respostas Meta. *(⚠️ Ativo apenas em modo controlado/DRY_RUN/handoff. Resposta real automática via Meta não está liberada sem App Review e validação final de permissões).*

## 5. Prospecção Manus (Corporativo e Condominial)
- 🟢 **130_MANUS_PROSPECCAO_B2B_RJ:** Identificação de alvos comerciais no RJ.
...
- Escrita/publicação: **Não homologada**.
...
- ⏸️ **081_PUBLICADOR_INSTAGRAM_META_API:** Worker de disparo.
- ⏸️ **082_PUBLICADOR_FACEBOOK_META_API:** Worker de disparo.
...
**Próxima Fase Prioritária (Novo Motor de Orçamento):**
*Integração de Fluxo:* Site DL_SITE_B2B → Formulário Avaliação Técnica V2 → `/webhook/dl-receptor` ou novo `/webhook/orcamento-v2` → `060_ORCAMENTO_RECEPCAO_SITE_DL`.
...
*(Regra Operacional Obrigatória: `numero_unidades` é estritamente obrigatório para o cálculo de rateio. Se faltar, bloquear o cálculo de valor por unidade).*

## 9. Site e Formulário de Orçamento
**Status do Site:** Localizado (`d:\AntiGravity\projeto_01\DL_SITE_B2B`).
- **Tecnologia:** HTML puro + CSS + Vanilla JS.
- **Home:** `index.html`
- **Webhook Principal:** `POST /webhook/dl-receptor`

**Definição dos Formulários:**
- **Formulário Atual:** Focado apenas em captação de lead genérico. **Manter o formulário atual como fallback até a V2 ser exaustivamente testada.**
- **Formulário V2:** O verdadeiro formulário de Avaliação Técnica / Motor de Orçamento pesado.

**Campos Consolidados (Formulário #dl-contact-form):**
- *Atuais:* `nome`, `whatsapp`, `email`, `nome_empresa_ou_condominio`, `tipo_cliente`, `cliente_ativo`, `bairro`, `cidade`, `servico_interesse`, `urgencia`, `melhor_horario`, `descricao`, `aceite_contato`, `aceite_lgpd`.
- *Faltantes (Para V2):* `cnpj`, `cpf`, `cep`, `endereco_completo`, `numero_unidades`, `numero_blocos`, `upload_video`, `upload_imagem`, `tipo_orcamento`, `responsavel_aprovacao`.

## 10. Google Drive e Agente Zelador
- ⏸️ **140_ZELADOR_MIDIAS_GOOGLE_DRIVE:** Pausado e preservado exclusivamente como base de auditoria/descoberta/sugestão, sem nenhuma execução destrutiva.
- 🔵 **140B_AGENTE_ZELADOR_EXECUTOR_MOVIMENTACAO_APROVADA:** Será o executor seguro, inicialmente blindado com a trava `GOOGLE_DRIVE_EXECUTOR_DRY_RUN=true`.

## 11. GitHub e Versionamento
**Status:** 🔴 CRÍTICO / PARCIALMENTE RESOLVIDO
- Um push limpo não substitui a necessidade de rotação de credenciais. Todos os tokens/chaves que foram expostos devem ser obrigatoriamente invalidados e recriados nos provedores oficiais. Não considerar esse problema resolvido até obter um push limpo sem GH013 E comprovar que as credenciais foram giradas nas fontes.
