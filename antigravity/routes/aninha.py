...
def triagem_lead(lead: LeadRequest, bg_tasks: BackgroundTasks):
    ...
    ⚡ Bolt Optimization: Using 'def' instead of 'async def' because the underlying
    call aninha.fazer_triagem() invokes a synchronous OpenAI API request. Using 'def'
    allows FastAPI to offload execution to a threadpool, preventing the event loop from blocking.
    """
    try:
