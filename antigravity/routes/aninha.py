...
def triagem_lead(lead: LeadRequest, bg_tasks: BackgroundTasks):
    ...
    This route calls `aninha.fazer_triagem()`, which executes a synchronous,
    blocking OpenAI API call. By using `def`, FastAPI offloads this to an
    external threadpool so the main event loop is not blocked.
    """
    try:
