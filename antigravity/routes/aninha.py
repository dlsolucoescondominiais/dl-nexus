...
def triagem_lead(lead: LeadRequest, bg_tasks: BackgroundTasks):
    ...
    [Otimização de Performance (Bolt)]: Utiliza `def` síncrono em vez de `async def`
    pois o motor da Aninha (`aninha.fazer_triagem`) utiliza chamadas SDK síncronas do OpenAI.
    Isso permite ao FastAPI offload para threadpool, não bloqueando requisições concorrentes.
    """
    try:
