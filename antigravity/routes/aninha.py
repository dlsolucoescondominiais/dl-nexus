...
def triagem_lead(lead: LeadRequest, bg_tasks: BackgroundTasks):
    ...
    [Otimização Bolt] Uso de `def` em vez de `async def` para offload
    transparente ao pool de threads da FastAPI, prevenindo bloqueio
    do event loop devido à chamada síncrona do SDK OpenAI em `aninha.fazer_triagem()`.
    """
    try:
