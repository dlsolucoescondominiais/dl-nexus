...
def triagem_lead(lead: LeadRequest, bg_tasks: BackgroundTasks):
    """
    ⚡ Bolt: Usando `def` em vez de `async def` porque a função executa `aninha.fazer_triagem()`
    que faz chamadas síncronas bloqueantes à OpenAI. O FastAPI envia funções `def` para uma
    threadpool externa, prevenindo que o main loop trave durante a espera da API.
