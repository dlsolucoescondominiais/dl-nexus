...
def triagem_lead(lead: LeadRequest, bg_tasks: BackgroundTasks):
    ...
    Changed `async def` to `def` because `aninha.fazer_triagem` uses the
    synchronous `openai` client. Using `def` allows FastAPI to automatically
    offload this blocking operation to an external threadpool, preventing
