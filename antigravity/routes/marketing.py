...
def aprovar_post(request: PostApprovalRequest):
    ...
    ⚡ Bolt Optimization: Using 'def' instead of 'async def' because requests.post
    is a synchronous, blocking network call. This allows FastAPI to offload execution
    to an external threadpool and prevents blocking the main asyncio event loop.
    """
    try:
        headers = {
