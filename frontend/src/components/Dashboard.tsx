...
export default function Dashboard() {
  ...
  // ⚡ Bolt: Calculate KPIs using useMemo with a single reduce pass
  // to avoid redundant O(N) filter loops and unnecessary re-renders.
  const kpis = useMemo(() => {
    return leads.reduce((acc, lead) => {
      acc.total_leads += 1;
      if (lead.pipeline_stage === 'negociacao') acc.em_negociacao += 1;
      else if (lead.pipeline_stage === 'fechado_ganho') acc.fechado_ganho += 1;
      else if (lead.pipeline_stage === 'contrato_recorrente') acc.contratos_ativos += 1;
      return acc;
    }, {
      total_leads: 0,
      em_negociacao: 0,
