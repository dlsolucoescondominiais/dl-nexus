...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    // O(N) single-pass aggregation to replace multiple O(N) filter().length operations
    const kpiCounts = (leadsData || []).reduce((acc, l) => {
      if (l.pipeline_stage === 'negociacao') acc.negociando++;
      else if (l.pipeline_stage === 'fechado_ganho') acc.ganhos++;
      else if (l.pipeline_stage === 'contrato_recorrente') acc.recorrentes++;
      return acc;
    }, { negociando: 0, ganhos: 0, recorrentes: 0 });

    setKpis({
      total_leads: leadsData?.length || 0,
      em_negociacao: kpiCounts.negociando,
      fechado_ganho: kpiCounts.ganhos,
      contratos_ativos: kpiCounts.recorrentes
    });
