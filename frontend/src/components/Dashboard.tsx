...
export default function Dashboard() {
  ...
  // Calculate KPIs using useMemo with a single loop pass over the `leads` state.
  // This prevents redundant re-renders from maintaining a secondary state,
  // and eliminates the multiple `.filter()` passes that loop over the array multiple times.
  const kpis = useMemo(() => {
    return leads.reduce(
      (acc, lead) => {
        acc.total_leads += 1;
        if (lead.pipeline_stage === 'negociacao') {
          acc.em_negociacao += 1;
        } else if (lead.pipeline_stage === 'fechado_ganho') {
          acc.fechado_ganho += 1;
        } else if (lead.pipeline_stage === 'contrato_recorrente') {
          acc.contratos_ativos += 1;
        }
        return acc;
      },
      {
