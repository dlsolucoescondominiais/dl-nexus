...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    // Calcula KPIs do Funil (Otimizado: O(N) em única passagem com reduce)
    const initialKpis = { total_leads: 0, em_negociacao: 0, fechado_ganho: 0, contratos_ativos: 0 };
    const calculados = (leadsData || []).reduce((acc, l) => {
      acc.total_leads++;
      if (l.pipeline_stage === 'negociacao') acc.em_negociacao++;
      if (l.pipeline_stage === 'fechado_ganho') acc.fechado_ganho++;
      if (l.pipeline_stage === 'contrato_recorrente') acc.contratos_ativos++;
      return acc;
    }, initialKpis);
