...
export default function Dashboard() {
  ...
  // ⚡ Bolt: Derived metrics single-pass calculation (O(N) Loop Fusion)
  // Replaces multiple sequential .filter() loops and prevents redundant re-renders
  const kpis = useMemo(() => {
    return leads.reduce(
      (acc, lead) => {
        acc.total_leads += 1;
        if (lead.pipeline_stage === 'negociacao') acc.em_negociacao += 1;
        if (lead.pipeline_stage === 'fechado_ganho') acc.fechado_ganho += 1;
        if (lead.pipeline_stage === 'contrato_recorrente') acc.contratos_ativos += 1;
        return acc;
      },
      { total_leads: 0, em_negociacao: 0, fechado_ganho: 0, contratos_ativos: 0 }
    );
  }, [leads]);
