...
export default function Dashboard() {
  ...
  // Optimizing KPIs: Calculate derived state using a single-pass loop (O(N)) over leads
  // instead of multiple filter/length passes, and use useMemo dependent on leads
  // to avoid manually synchronizing secondary state.
  const kpis = useMemo(() => {
    let em_negociacao = 0;
    let fechado_ganho = 0;
    ...
    for (let i = 0; i < leads.length; i++) {
      ...
      if (stage === 'negociacao') {
        em_negociacao++;
      } else if (stage === 'fechado_ganho') {
        fechado_ganho++;
      } else if (stage === 'contrato_recorrente') {
        contratos_ativos++;
      }
    }
