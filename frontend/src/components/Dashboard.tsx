...
export default function Dashboard() {
  ...
  // Calculate derived KPIs using useMemo instead of manual sync with a secondary useState.
  // Combines 3 O(N) array passes (filter) into a single O(N) loop fusion for efficiency.
  const kpis = useMemo(() => {
    let em_negociacao = 0;
    let fechado_ganho = 0;
